...
@jim_cmd(pattern="addecho(?: |$)(.*)")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await event.edit("`Balas pesan Pengguna untuk menggemakan pesannya`")
    jimevent = await event.edit("`Tambahkan Echo ke pengguna...`")
    user, rank = await get_user_from_event(event, jimevent, nogroup=True)
    if not user:
        return
...
async def echo(event):
    ...
    if is_echo(chat_id, user_id):
        try:
            ...
            await edit_delete(jimevent, f"**Error:**\n`{str(e)}`")
        else:
